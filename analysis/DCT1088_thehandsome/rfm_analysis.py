...
def prep_df(df):
    total_df_identified = df.loc[df['unique_user_id'].notnull()]
    total_df_identified.loc[total_df_identified['event_name']=='af_content_view', 'content_type'] = \
        get_event_from_values(np.array(total_df_identified.loc[total_df_identified['event_name']=='af_content_view', 'event_value']), 'af_content_type')
    total_df_identified['content_type'] = total_df_identified['content_type'].fillna('')
    total_df_identified['content_type'] = total_df_identified['content_type'].apply(lambda x: '/'.join(str(x).split('/')[:2]))

    total_df_identified.loc[total_df_identified['event_name']=='af_content_view', 'content'] = \
        get_event_from_values(np.array(total_df_identified.loc[total_df_identified['event_name']=='af_content_view', 'event_value']), 'af_content')
    total_df_identified['is_outlet_product'] = total_df_identified['content'].fillna('').str.slice(start=3, stop=4).apply(
    ...
    total_df_identified['brand_name'] = total_df_identified['content'].fillna('').str.slice(stop=2).apply(lambda x: brand_dict[x])
    total_df_identified = total_df_identified.drop(columns=['content'])
    return total_df_identified
