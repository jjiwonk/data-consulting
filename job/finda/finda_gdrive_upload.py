...
class raw_data :
    ...
    def get_index_sheet(self):
        sheet_url = 'https://docs.google.com/spreadsheets/d/1nN-A0_f1ZpeUSS5JEibGcKKkoTgdgibGStTt_qZkdh0/edit#gid=0'
        index_sheet = google_drive.GoogleDrive().get_work_sheet(url = sheet_url, sheet_name = 'INDEX')
        index_df = google_drive.GoogleDrive().sheet_to_df(sheet = index_sheet)

        return index_df

    def prep(self, df, index_df):
...
if __name__ == "__main__":
    ...
    index_df = raw_data().get_index_sheet()
    merge_df = raw_data().prep(df, index_df)
