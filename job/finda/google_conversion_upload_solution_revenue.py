...
if __name__ == "__main__":
    ...
    def data_prep():
        ...
        df = conversion_id(df, 'loan_contract_completed_fee', 'android', '6469871207')
        df = conversion_id(df, 'loan_contract_completed_fee', 'ios', '6469884678')
        ...
        #revenue 가공 #수정
        df.loc[df['event_name'] != 'loan_contract_completed_fee','event_value'] = 0
        df['event_value'] = df['event_value'].apply(lambda x: eval(x)if x != 0 else 0)
        df['event_value'] = df['event_value'].apply(lambda x: x.get('af_revenue') if x != 0 else 0)
