...
def event_schedule_prep():
    ...
    event_df = event_schedule.pivot_table(index='date', columns='channel', values=['event_dummy', 'max_discount'], aggfunc='mean').reset_index()
    event_df = event_df.fillna(0)
    event_df['max_discount'] = event_df[event_df.columns[6:]].sum(axis=1)
    event_df = event_df.drop(event_df[event_df.columns[6:-1]], axis=1)
    event_df.columns = ['date', 'ssgonline_event', 'oliveyoung_event', 'company_event', 'kakaogift_event', 'all_event', 'max_discount']
    event_df[event_df.columns[1:-1]] = event_df[event_df.columns[1:-1]].astype('int')
...
def sales_data_prep():
    ...
    # 전체 통합
    date_df = pd.DataFrame({'date': [date.strftime("%Y-%m-%d") for date in pd.date_range('2022-01-01', '2023-03-07')]})
