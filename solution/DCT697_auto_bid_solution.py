        ...
        for index, row in self.bid_adjust_df.iterrows():
            ...
            if goal_rank < cur_rank:
                ...
                if cur_bid > max_bid:
                    if cur_rank == 30:
                        cur_rank = '-'
                    max_bid_msg.append(f"현재 {cur_bid}원, 최대 {max_bid}원 /// {campaign}, {adgroup}, {keyword}: {cur_rank}위")
                    bid_amt = max_bid
                ...
                # 현재 순위가 목표 순위 보다 높은 경우
                    ...
                    if cur_bid < min_bid:
                        min_bid_msg.append(f"현재 {cur_bid}원, 최소 {min_bid}원 /// {campaign}, {adgroup}, {keyword}: {cur_rank}위")
                        bid_amt = cur_bid
                        data = dict(nccKeywordId=keyword_id, nccAdgroupId=group_id, bidAmt=bid_amt, useGroupBidAmt=use_gbamt)
                        self.data_list.append(data)
